...
const process_cli = function(argv_vals){
  ...
  const run_ffmpeg_conversion_video = function(){
    return new Promise((resolve, reject) => {
      ...
      let manifest
      manifest = path.join(argv_vals["--directory-prefix"], 'master.m3u8')
      if (! fs.existsSync(manifest)) {
        manifest = path.join(argv_vals["--directory-prefix"], 'video.m3u8')
      }
      if (! fs.existsSync(manifest)) {
      ...
      const cmd = `ffmpeg ${log} -allowed_extensions ALL -i "${manifest}" -c copy -movflags +faststart "${argv_vals["--mp4"]}"`
