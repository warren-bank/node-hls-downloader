...
const process_cli = function(argv_vals){
    ...
    return new Promise((resolve, reject) => {
      ...
      if ((typeof manifest === 'string') && (manifest.length)) {
        ...
        for (i=0; i < lines.length; i++) {
          ...
          if (match === null)
            continue
          url = url_resolve(base_url, match[1])
  ...
  const add_data_url = function(needle, haystack){
    for (let i=0; i < haystack.length; i++) {
      if (needle.url === haystack[i].url) return haystack[i]
    }
  ...
  const process_stream_manifest_data = function(manifest_data, base_url, file_ext, ...sub_directory){
    ...
    const data_urls    = []
    const key_urls     = []
    let data_url, data_name, duplicate
    let key_url, key_name

    lines.forEach(line => {
      line = line.trim()
      if (line.length && (line[0] !== '#')) {
        data_url = url_resolve(base_url, line)
        data_name = `segment_${data_urls.length}.${file_ext}`
        duplicate = add_data_url({url: data_url, name: data_name}, data_urls)

        if (duplicate)
          data_name = duplicate.name
        ...
        if (match !== null) {
          key_url  = url_resolve(base_url, match[2])
  ...
  const process_stream = async function(stream, manifest, base_url){
    ...
    if (!argv_vals["--skip-video"]) {
      url = url_resolve(base_url, stream.url)
